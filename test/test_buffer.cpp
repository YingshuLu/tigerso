...
#include "mcutil.h"
#include "McDefines.h"
#include "McHttpSession.h"
#include "McHttpParser.h"
    ...
    while(true)
    {
        ...
        for (int i = 0; i<num; i++) {
            ...
            if(events[i].data.fd == listen_sock.get_socket()) {
                ...
                McSocketPtr client_sock = sock_pool.take();
            ...
            if((events[i].events & EPOLLIN) && events[i].data.fd != listen_sock.get_socket()) {
                ...
                McHttpSession hs(client_sock);
                McHttpParser::parser_http(hs);
                string host = hs.get_header_value_by_field("host");
