...
static const int DaemonSignals[] = {
    SIGCHLD,
    SIGSEGV,
    SIGTERM,
    SIGUSR1
...
int DaemonBase::start(const int port) {
    if(checkOldDaemon() != 0) { 
    ...
    if(createMasterListening(host, port) != 0) {
        INFO_LOG("Error: failed to create listening socket, %s:%d, reason: %s", host.c_str(), port, strerror(errno));
        exit(EXIT_FAILURE);
    }
    return parentStart();
}
...
int DaemonBase::initilize() {
    master_ = std::shared_ptr<Socket>(new Socket());
    loadConfigParams();
...
int DaemonBase::setupInterestedSignal() {
    struct sigaction sa;
    for(int i = 0; i < sizeof(DaemonSignals)/sizeof(int); i++) {
        if(SysUtil::set_signal_handler(DaemonSignals[i], __signalHandle) == SIG_ERR) {
...
int DaemonBase::createMasterListening(const std::string& ipaddr, const int port) {
    char portstr[12] = {0};
...
int main() {
    DaemonBase daemon("test-daemon");
    printf("DaemonBase start\n");
    daemon.start();
    printf("DaemonBase done\n");
    return 0;
}
